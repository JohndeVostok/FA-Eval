...
class DummyFA(Function):
    ...
    def forward_func(q, k, v, num_group, scale, dropout_p, mask):
        ...
        m = s.max(dim=-1, keepdim=True)[0].to(hp_type)
        p = torch.exp(s - m).to(lp_type)
        l = p.sum(dim=-1, keepdim=True, dtype=hp_type)
        p = p / (1 - dropout_p)
        L = m + torch.log(l)
        o = (torch.matmul(p, v) / l).to(lp_type)
        return o, L
    ...
    def backward_func(do, q, k, v, o, L, num_group, scale, dropout_p, mask):
        if num_group > 1:
            k = k.repeat_interleave(num_group, dim=1)
            v = v.repeat_interleave(num_group, dim=1)
        s = torch.matmul(q, k.transpose(-2, -1)).to(hp_type) / scale
        if mask is not None:
            s = s.masked_fill(mask == 0, float('-inf'))
        p = torch.exp(s - L).to(lp_type)
        dv = torch.matmul((p / (1 - dropout_p)).transpose(-2, -1), do)
        dp = torch.matmul(do, v.transpose(-2, -1)) / (1 - dropout_p)
        D = torch.sum(do * o, dim=-1, keepdim=True)
        ds = p * (dp - D)
        dq = (torch.matmul(ds, k) / scale).to(lp_type)
        dk = (torch.matmul(ds.transpose(-2, -1), q) / scale).to(lp_type)
    ...
    def forward(ctx, q, k, v, num_q_heads, num_kv_heads, dropout_p, training, mask):
        ...
        output_reshaped, L = DummyFA.forward_func(q_reshaped, k_reshaped, v_reshaped, num_groups, scale, dropout_p, mask)
    ...
    def backward(ctx, grad_output):
        ...
        dq, dk, dv = DummyFA.backward_func(grad_output, q_reshaped, k_reshaped, v_reshaped, output_reshaped, L, num_groups, scale, dropout_p, mask)
