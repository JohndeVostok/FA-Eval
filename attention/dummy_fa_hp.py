...
class DummyFAHP(Function):
    ...
    def forward_func(q, k, v, num_group, scale, dropout_p, mask):
        ...
        p = p.to(lp_type)
        L = m + torch.log(l)
        o = (torch.matmul(p, v) / (l * (1 - dropout_p))).to(hp_type)
        return o, L
    ...
    def forward(ctx, q, k, v, num_q_heads, num_kv_heads, dropout_p, training, mask):
        ...
        output_reshaped, L = DummyFAHP.forward_func(q_reshaped, k_reshaped, v_reshaped, num_groups, scale, dropout_p, mask)
    ...
    def backward(ctx, grad_output):
        ...
        if DummyFAHP.hp_o is not None:
            output_reshaped = DummyFAHP.hp_o.view(batch_size, seq_len, num_q_heads, head_dim).transpose(1, 2)
            print("use hp cpu output.")

        dq, dk, dv = DummyFAHP.backward_func(grad_output, q_reshaped, k_reshaped, v_reshaped, output_reshaped, L, num_groups, scale, dropout_p, mask)
